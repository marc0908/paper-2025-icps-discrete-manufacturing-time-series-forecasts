...
from ray.tune import Tuner, TuneConfig, PlacementGroupFactory, RunConfig
...
def run_hyperparameter_optimization(
    ...
    def training_wrapper(selected_hyperparams):
        metrics_history = []
        ...
        def callback_wrapper(val_loss, val_loss_min, model, epoch=0, extras: dict | None = None):
            # Standard-Report
            result = callback(val_loss, val_loss_min, model, epoch, metrics_history, extras)
    ...
    # Print best results
    best_trial = analysis._experiment_analysis.get_best_trial("val_loss", "min", "last") # TODO: Use get_best_result
    if best_trial:
